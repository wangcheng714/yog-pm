...
exports.registry = function(commander){
    ...
    function startOrReload(configFile){
        exec("ps aux | grep 'pm2: Daemon'", function(error, stdout, stderr){
            ...
            if(stdout !== null){
                //todo ： 重写采用\n切分
                //example : http://cnodejs.org/topic/51514f09604b3d5121204c89
                //todo : 使用linux工具包
                //href : https://github.com/robertkeizer/linux-util
				...
				console.log(config);
                        if(config[0]){
                            //优先使用用户设置的pm_bin，没有设置则使用全局
                            var command = config[0]["pm_bin"] ?
                                config[0]["pm_bin"] + "/pm2 start " + configFile : "pm2 start " + configFile;
